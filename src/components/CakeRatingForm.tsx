...
export function CakeRatingForm({ cakeId, onRatingSubmitted }: CakeRatingFormProps) {
    ...
    useEffect(() => {
        if (session?.session?.user?.id && ratings) {
            const userRating = ratings.find(rating => rating.user_id === session.session?.user?.id);
            if (userRating) {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.message || 'Une erreur est survenue lors de l\'enregistrement de votre note');
        }
    };
