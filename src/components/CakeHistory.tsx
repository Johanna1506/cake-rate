...
const CakeRatings: React.FC<{ cakeId: string }> = ({ cakeId }) => {
    ...
    const averageAppearance = ratings.reduce((acc, rating) => acc + rating.appearance, 0) / ratings.length;
    const averageTaste = ratings.reduce((acc, rating) => acc + rating.taste, 0) / ratings.length;
    const averageTheme = ratings.reduce((acc, rating) => acc + rating.theme_adherence, 0) / ratings.length;
...
export const CakeHistory: React.FC = () => {
    const { data: cakes, isLoading: cakesLoading, error: cakesError } = useCakes();
