...
export const CakeUpload: React.FC<CakeUploadProps> = ({ onClose, weekId }) => {
  ...
  const validateForm = () => {
    ...
    if (description && description.length < 10) {
      setDescriptionError(
        "Si vous ajoutez une description, elle doit contenir au moins 10 caractères"
      );
      isValid = false;
    } else {
      setDescriptionError("");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabaseServer.storage.from("cakes").getPublicUrl(filePath);

      const { error: insertError } = await supabaseServer.from("cakes").insert({
  ...
  useEffect(() => {
    if (
      !weekLoading &&
      currentWeek &&
      currentWeek.user_id !== session?.session?.user?.id
    ) {
      navigate("/");
    }
  ...
  if (!session?.session?.user) {
    return (
      <Box sx={{ textAlign: "center", p: 4 }}>
        <Typography>
          Vous devez être connecté pour ajouter un gâteau.
        </Typography>
      </Box>
    );
  }
  ...
  if (!currentWeek) {
    return (
      <Box sx={{ textAlign: "center", p: 4 }}>
        <Typography color="info">Aucune semaine active en cours.</Typography>
      </Box>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Box
        ...
        role="form"
        aria-label="Formulaire d'ajout de gâteau"
      >
        <Box
          sx={{
            mb: { xs: 2, sm: 3 },
        ...
        {previewUrl && (
          <Box
            ...
            role="img"
            aria-label="Aperçu de la photo du gâteau"
          >
            <Box
              component="img"
              src={previewUrl}
        ...
        >
          ...
          <Button
            ...
            disabled={loading}
            aria-label={
              loading ? "Ajout du gâteau en cours..." : "Ajouter le gâteau"
            }
            sx={{
              py: { xs: 1, sm: 1.5 },
              fontSize: { xs: "0.875rem", sm: "1rem" },
