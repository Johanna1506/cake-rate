...
function PrivateRoute({ children, requireAdmin = false }: { children: React.ReactNode, requireAdmin?: boolean }) {
  ...
  const { data: userDetails, isLoading: userDetailsLoading } = useUserDetails(session?.session?.user?.id || '');
...
function AppContent() {
  const { data: session, isLoading: sessionLoading } = useSession();
  const { isLoading: userDetailsLoading } = useUserDetails(session?.session?.user?.id || '');
  ...
  return (
    <>
      ...
      <Routes>
        ...
        <Route path="/cake-upload" element={
          <PrivateRoute>
            <UploadCake />
          </PrivateRoute>
        } />
        <Route path="/rate/:cakeId" element={
          <PrivateRoute>
            <RateCake />
          </PrivateRoute>
        } />
        <Route path="/cake-history" element={
          <PrivateRoute>
            <CakeHistory />
          </PrivateRoute>
        } />
